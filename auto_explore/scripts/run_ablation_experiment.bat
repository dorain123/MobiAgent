...
if "%~1"=="" (
    echo Usage: run_ablation_experiment.bat EXPERIMENT_NAME
    echo Example:
    echo   run_ablation_experiment.bat E0_full
    pause
    exit /b 1
)
...
rem Auto-search single-experiment ablation template for Windows.
rem Set AUTO_EXPLORE_APP_NAME in the shell before running, or keep the default ASCII app name.
rem
rem Privacy note:
...
echo Running ablation experiment=%EXPERIMENT_NAME% app=%APP_NAME% depth=%DEPTH% breadth=%BREADTH% repeats=%REPEATS% ...
